...
export function loadPly(buffer: ArrayBuffer): SplatData {
  ...
  const maxVertices = Math.floor(availableBytes / stride);
  if (maxVertices < vertexCount) {
    console.warn(`PLY truncated: header says ${vertexCount} vertices but data fits ${maxVertices}. Using ${maxVertices}.`);
